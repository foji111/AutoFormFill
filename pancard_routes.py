...
async def extract_pan_from_file(file: UploadFile = File(...)):
    ...
    try:
        data = extract_pancard_info_from_image(img)
        return PANCardData(**data)
    except Exception as e:
        # Log the error safely here (e.g. print(e))
        print(f"Error processing PAN card: {e}")
        # Return sanitized error
        raise HTTPException(status_code=500, detail="An error occurred processing the image.")
